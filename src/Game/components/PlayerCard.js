    ...
    >
      <div className="flex justify-between items-center">
        <h3 className="text-sm font-bold flex items-center">
          <User
            className={`mr-1 ${onCourt ? "text-green-700" : "text-gray-500"}`}
            size={16}
          />
          {player.name}
        </h3>
        ...
        <p className="text-xs font-semibold text-blue-700">
          {onCourt ? 'Current:' + formatTime(currentTime) : '' }
        </p>

      <p className="text-xs">Total Active: {formatTime(totalActiveTime)}</p>
      <p className="text-xs">Total Benched: {formatTime(totalBenchedTime)}</p>
